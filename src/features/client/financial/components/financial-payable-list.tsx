...
import {
  IconDots, IconCash, IconHistory, IconAlertCircle, IconCheck, IconSearch, IconTrendingDown
...
import { FinancialListFilters } from './financial-list-filters';
import { getFinancialTitles } from '../api/financial.api';
import { PAYMENT_METHOD_LABELS } from '../constants/financial-constants';
import { FinancialFilterValues, FinancialTitle } from '../types';
import { DataGrid } from '@/shared/components/ui/data-grid/data-grid';

export function FinancialPayableList() {
  ...
  const { data: titles = [], isLoading } = useQuery({
    queryKey: ['financial-titles', 'PAYABLE', serializedFilters],
    queryFn: () => {
      return getFinancialTitles({
        type: 'PAYABLE', // <--- FILTRO PRINCIPAL
  ...
  const stats = useMemo(() => {
    const totalPayable = titles.reduce((acc, t) =>
    ...
    return { totalPayable, totalOverdue, overdueCount: overdueTitles.length, totalPaid };
  }, [titles]);
  ...
  const filteredData = useMemo(() => {
    ...
    return titles.filter(item =>
      ...
      item.supplier?.name.toLowerCase().includes(lower) // Busca por Fornecedor
    );
  }, [titles, localSearch]);
  ...
  const columns = useMemo<ColumnDef<FinancialTitle>[]>(
    () => [
      ...
      {
        accessorKey: 'supplier.name', // Alterado de customer para supplier
        header: 'Fornecedor',
        cell: ({ getValue }) => <Text size="sm" fw={500}>{getValue() as string || 'N/A'}</Text>
      },
      {
        accessorKey: 'dueDate',
        header: 'Vencimento',
        cell: ({ row }) => {
          ...
          return (
            <Text size="sm" c={isOverdue ? 'red' : undefined} fw={isOverdue ? 700 : 400}>
              ...
              {isOverdue && <IconAlertCircle size={12} style={{ marginLeft: 4, display: 'inline' }} />}
            </Text>
          );
        }
      },
        ...
        cell: ({ row }) => (
          <Group gap={0} justify="flex-end">
            {row.original.status !== 'PAID' && row.original.status !== 'CANCELED' && (
              <Button
                size="compact-xs"
                color="orange" // Cor diferente para diferenciar de Recebimento
                variant="light"
                mr="xs"
  ...
  return (
    <Flex direction="column" gap="md">
      <SimpleGrid cols={{ base: 1, sm: 3 }}>
        <Paper withBorder p="md" radius="md">
          <Group justify="space-between">
            <Text size="xs" c="dimmed" fw={700}>A Pagar (Total)</Text>
            <ThemeIcon variant="light" color="orange" radius="xl"><IconTrendingDown size={16} /></ThemeIcon>
          </Group>
          <Text size="xl" fw={700} mt="sm">{formatCurrency(stats.totalPayable)}</Text>
        </Paper>
        ...
        <Paper withBorder p="md" radius="md">
          <Group justify="space-between">
            <Text size="xs" c="dimmed" fw={700}>Pago (Filtro)</Text>
      ...
      <TextInput
        placeholder="Buscar por Fornecedor ou Nº Título..."
