...
export function OrderSummary() {
  ...
  const createOrderMutation = useMutation({
    mutationFn: async (paymentData: any) => {
      // Normalização do Método de Pagamento para o padrão do Backend (Enum)
      const methodNormalized = paymentData.method ? paymentData.method.toUpperCase() : 'CASH';
      ...
      const payload = {
        ...
        // Dados vindos do Modal de Pagamento
        paymentMethod: methodNormalized, // 'WALLET', 'CREDIT_CARD', 'CASH'
        installments: paymentData.installments, // Array de parcelas (se houver)

        // Mapeia itens do carrinho para o formato da API
        items: items.map(i => ({
